...
color = [[49,255,180]*25]
nocolor = [item for sublist in color for item in sublist]
...
results = model.predict(source="4", show=True, stream=True)
for result in results:
    ...
    try:
        ...
        ser.write(b'\xff')
        ser.write(bytes(nocolor + servoPositions(coordList)))
        # servoPositions(coordList)
    except IndexError:
